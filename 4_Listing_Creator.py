...
def process_listings(batch_size: int = 10) -> None:
    """Process listings and create WordPress posts."""
    try:
        while True:
            # Get batch of unprocessed listings
            listings = execute_query("""
                SELECT listing_id, post_title, post_content, post_category,
                       business_hours, website, phone, email, facebook, instagram,
                       what_makes_us_different, street, city, region, country, zip,
                       latitude, longitude, post_images
                FROM listings
        ...
        logger.error(f"Error in process_listings: {str(e)}", exc_info=True)
        raise
